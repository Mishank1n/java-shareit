...
public class ErrorHandler {
    ...
    @ExceptionHandler
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ExceptionResponse handlerWithoutRequestParam(MissingServletRequestParameterException e) {
        return new ExceptionResponse("Нельзя выполнить данный запрос без параметра запроса");
    }

    @ExceptionHandler
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ExceptionResponse handlerNotValidBadRequest(MethodArgumentNotValidException e) {
        return new ExceptionResponse(Objects.requireNonNull(e.getBindingResult().getFieldError()).getDefaultMessage());
    }
}
